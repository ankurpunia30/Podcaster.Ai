...
export default function EpisodeForm() {
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/ai/generate-podcast`, {
        topic: aiTopic,
        genre: form.genre,
      ...
      if (response.data) {
        ...
        setTimeout(() => {
          setCurrentStep(3) // Move to voice selection
        }, 1000)
      }
    } catch (error) {
  ...
  const generatePodcast = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/tts/create-podcast`, payload)
